...
`-s` 是 ssserver 的监听地址。`0.0.0.0` 只监听 IPv4；如果服务器有公网 IPv6，把它换成 `-s ::` 即可让**同一个** ssserver 同时监听 v4 和 v6（Linux 下 `::` 默认是双栈 socket，不需要跑两个进程）——`install.sh` 首次安装时会自动探测服务器是否有全局 IPv6 地址并据此选择。
